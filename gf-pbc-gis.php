...
function sflwa_pbc_handle_bulk_action( $action, $entries, $form_id ) {
    ...
    // Ensure the addon class is initialized before trying to use it
    sflwa_pbc_init_addon();
...
// 3. THE PLUGIN LOADER
add_action( 'gform_loaded', 'sflwa_pbc_init_addon', 5 );

function sflwa_pbc_init_addon() {
    if ( ! method_exists( 'GFForms', 'include_addon_framework' ) ) return;
    
    // Only define the class if it hasn't been defined yet and the framework is present
    if ( ! class_exists( 'SFLWAPBCAddOn' ) ) {
